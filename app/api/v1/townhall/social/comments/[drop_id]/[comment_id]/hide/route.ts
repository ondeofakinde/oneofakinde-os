...
export async function POST(
  request: Request,
  context: RouteContext<CommentHideRouteParams>
) {
  ...
  const result = await commerceBffService.hideTownhallComment(
  ...
  if (!result.ok) {
    return result.reason === "forbidden"
      ? forbidden("not allowed to hide this comment")
