...
type CommentRestoreRouteParams = {
  drop_id: string;
  comment_id: string;
};
...
export async function POST(
  request: Request,
  context: RouteContext<CommentRestoreRouteParams>
) {
  ...
  const result = await commerceBffService.restoreTownhallComment(
  ...
  if (!result.ok) {
    return result.reason === "forbidden"
      ? forbidden("not allowed to restore this comment")
